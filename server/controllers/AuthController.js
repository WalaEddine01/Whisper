...
const exDate = 10000;
const createToken = (id) => jwt.sign({ id }, 'a sicrate to change latter', { expiresIn: exDate });
...
class AuthController {
  ...
  // static async signupGet(request, response) {
  ...
  static async loginPost(request, response) {
    ...
    try {
      ...
      response.status(400).json({ msg: err }); // i will wrie new error handler
    }
  }
}
